...
void Texture::renderText(const std::string& text, SDL_Color color , std::string filename, int size) {
	...
	//std::cout << (strcmp(TTF_GetError(), "Text has zero width") != 1)<<std::endl;
	if (surface == NULL ) {
		printf("Error in text render:%s\n", TTF_GetError());
		return ;
	}
	if (surface == NULL && (strcmp(TTF_GetError(), "Text has zero width") != 0)) {
		printf("Error in text render:%s\n", TTF_GetError());
		return ;
	}
	...
	SDL_SetRenderDrawColor(&Window::getInstance()->getRenderer(), 255, 255, 255, 0);
